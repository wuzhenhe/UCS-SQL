...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    assert len(plist) == len(glist), ("number of sessions must equal", len(plist), len(glist))
    ...
    out_evaluation_json_path = os.path.join(out_dir, "corre_evaluation.json" if "corre" in predict else "no_corre_evaluation.json")
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--gold', dest='gold', type=str, help="the path to the gold queries", default='./data/spider/dev_gold.sql')
    # parser.add_argument('--pred', dest='pred', type=str, help="the path to the predicted queries", default='xxxxx.sql')
    ...
    # parser.add_argument('--table', dest='table', type=str, help="the tables.json schema file",default='./data/spider/tables_train_dev.json')
    ...
    # test
    parser = argparse.ArgumentParser()
    parser.add_argument('--gold', dest='gold', type=str, help="the path to the gold queries", default='./data/spider/test_data/test_gold.sql')
    parser.add_argument('--pred', dest='pred', type=str, help="the path to the predicted queries", default='xxxxx.sql')
    parser.add_argument('--db', dest='db', type=str, help="the directory that contains all the databases and test suites",default='./data/spider/test_database')
    parser.add_argument('--table', dest='table', type=str, help="the tables.json schema file",default='./data/spider/test_data/tables_test.json')
    parser.add_argument('--etype', dest='etype', type=str, default='all',
                        help="evaluation type, exec for test suite accuracy, match for the original exact set match accuracy",
                        choices=('all', 'exec', 'match'))
    parser.add_argument('--plug_value', default=False, action='store_true',
